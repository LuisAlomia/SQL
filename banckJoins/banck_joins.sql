...
select account_no, client_id, name from accounts as a left join account_types on a.id = type;

create view v_accounts_types as select account_no, client_id, name from accounts as a left join account_types on a.id = type;

select * from v_accounts_types;

-- 3) v_transactions_detail

select t.id, amount, tp.name from transactions as t left join transactions_types as tp on t.transaction_type = tp.id;

create view v_transactions_detail as select t.id, amount, tp.name from transactions as t left join transactions_types as tp on t.transaction_type = tp.id;

select * from v_transactions_detail;

-- 4) v_transations_types_detail

select t.id, amount, tp.name from transactions as t right join transactions_types as tp on t.transaction_type = tp.id;

create view v_transations_types_detail as select t.id, amount, tp.name from transactions as t right join transactions_types as tp on t.transaction_type = tp.id;
